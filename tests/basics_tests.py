...
class TestBasic(unittest.TestCase):
    ...
    def test_conv1d_adjoint_hard(self):
        ...
        conv = basics.Convolve1d(x.shape, weight, stride=2, padding=1, dilation=2, device=device)
        out = conv.adjoint(Ax)

        exp = F.conv_transpose1d(Ax, weight, stride=2, padding=1, dilation=2)
        assert(torch.allclose(out, exp, rtol=1e-3))
...
if __name__ == '__main__':
    ...
    # Ax = F.conv1d(x, weight, bias=bias, stride=2, padding=1, dilation=2)
    # conv = basics.Convolve1d(x.shape, weight, bias=bias, stride=2, padding=1, dilation=2, device=device)
    # print(conv.size_in, conv.size_out)
    # print(Ax.shape)
    # out = conv.adjoint(Ax)
